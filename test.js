...
var assert = require('assert'),
  ...
  CradleStore = require('./')(connect);

var store = new CradleStore() ,
  store_alt = new CradleStore({ db: 15 });

store.client.on('connect', function(){
  // #set()
  store.set('123', { cookie: { maxAge: 2000 }, name: 'trial' }, function(err, ok){
    ...
    store.get('123', function(err, data){
      ...
      assert.deepEqual({ cookie: { maxAge: 2000 }, name: 'trial' }, data);
        ...
        store_alt.length(function (err, len) {
          ...
          store.clear(function(err, ok){
            ...
            store.length(function(err, len){
              ...
              // #set null
              store.set('123', { cookie: { maxAge: 2000 }, name: 'trial' }, function(){
